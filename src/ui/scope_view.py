...
class ScopeView(QtWidgets.QWidget):
    ...
    def __init__(self, n_channels: int = 10, parent=None):
        """
        初始化示波器视图
        ...
        logger.info(f"示波器视图已初始化: {n_channels}通道")
    ...
    def set_vertical_scale(self, channel: int, div_value: float, offset: float = 0.0):
        """
        设置垂直缩放
